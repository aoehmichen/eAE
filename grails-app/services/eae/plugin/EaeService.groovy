...
class EaeService {
    ...
    def customPreProcessing(params, workflow, MONGO_URL,database,username){
        switch (workflow){
            case "CrossValidation":
                return cvPreprocessing(params, MONGO_URL,database,username);
            default:
     ...
     */
    private def cvPreprocessing(params, MONGO_URL, database, username){
        ...
        if( ((String)params.doEnrichment).toBoolean()){
            ...
            mongoDocumentIDPE = mongoCacheService.initJob(MONGO_URL, database, "PathwayEnrichment", "NoSQL", username, query )
            doEnrichement = "true"
        }
